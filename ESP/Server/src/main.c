...
#define CLIENT_ADDRESS "f0:f5:bd:01:96:0a"
...
void uart_task(void *pvParameters)
{
    ...
    while (1)
    {
        int len = uart_read_bytes(UART_PORT, data, BUFLEN, portMAX_DELAY); // 1s timeout
        if (len == BUFLEN)
        {
            if (g_conn_handle != BLE_HS_CONN_HANDLE_NONE && notify_enable)
            {
                struct os_mbuf *txom = ble_hs_mbuf_from_flat(data, len);
                if (txom && (0 == ble_gatts_notify_custom(g_conn_handle, ble_svc_gatt_read_val_handle, txom)))
                {
                    ESP_LOGI(TAG, "Notification sent successfully");
                }
                else
